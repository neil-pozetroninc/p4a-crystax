...
class Python2Recipe(TargetPythonRecipe):
    ...
    conflicts = ['python2', 'python3', 'python3crystax']
    ...
    def check_for_sslso(self, ssl_recipe, arch):
        ...
        if not os.path.exists(join(source_dir, 'libs', arch.arch, 'libssl.so')):
    ...
    def build_arch(self, arch):
        ...
        if self.from_crystax and 'openssl' in self.ctx.recipe_build_order:
            ...
            if stage < 3:
                ...
                #    shprint(sh.mv,join(openssl_build_dir,'{}{}.so'.format(lib,ssl_recipe.version)),
                #                  join(openssl_build_dir,'{}.so'.format(lib)))
